...
final class HtmlParserFuzzer {
  ...
  private static void parseOne(final byte[] bytes)
    ...
    Parser jsoupParser = null;
    Class clazz = null;
    try {
      clazz = Class.forName("org.apache.tika.parser.html.JSoupParser");
    } catch (ClassNotFoundException e) {
      // swallow and return
      return;
    }
    try {
      jsoupParser = (Parser) clazz.getDeclaredConstructor().newInstance();
    } catch (Exception e) {
      throw new RuntimeException("JSoup parser exists, "
          + "but something went horribly wrong");
    }
    Parser p = new AutoDetectParser(jsoupParser, new CompositeExternalParser());
