...
class ThreeDXMLParserFuzzer {
    ...
    private static void parseOne(byte[] bytes) throws TikaException, IOException, SAXException {
        ...
        Class clazz = null;
        try {
            clazz = Class.forName("org.apache.tika.parser.threedxml.ThreeDXMLParser");
        } catch (ClassNotFoundException e) {
            //swallow and return
            return;
        }
        try {
            p = (Parser) clazz.getDeclaredConstructor().newInstance();
        } catch (Exception e) {
            throw new RuntimeException("ThreeDXMLParser exists, but something went " +
                    "horribly wrong");
        }
